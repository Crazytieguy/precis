...
- **Non-deterministic output** — `HashMap` iteration order in `build_groups()` makes group indices arbitrary, and float priority comparison doesn't break ties deterministically. Output can vary between runs and between debug/release builds. Needs deterministic group ordering and integer or cross-multiplication priority comparison.
...
- **Deterministic output** — sort groups after building (deterministic group indices), and use integer arithmetic or cross-multiplication for priority comparison to avoid float non-determinism across optimization levels.
- **Performance** — fixture tests take ~9 minutes in debug mode, ~55 seconds in release. Most fixture snapshot tests are commented out until this improves. Run tests in release mode (`cargo test --release`) until debug performance is addressed. Re-introduce fixture snapshots progressively as performance improves.
...
- **Sibling-count value factor** — groups with many symbols should have lower per-symbol value. A file with 50 functions means each function is less important individually. The value computation already takes `&Group`, so adding `group.symbols.len()` as a factor is straightforward.

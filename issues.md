...
- Add more fixture snapshot tests — aiming for 5+ per language, currently 5 Rust (`either`, `anyhow`, `once_cell`, `thiserror`, `log`), 3 TypeScript (`neverthrow`, `ts-pattern`, `superstruct`), 3 JavaScript (`semver`, `dotenv`, `commander`), 3 TSX (`cmdk`, `react-hot-toast`, `sonner`)

...
- **Root-level fixture testing (partial)** — root-level fixture and budget tests added for 10 repos (either, neverthrow, pluggy, sonner, commander, anyhow, log, ts-pattern, typeguard, mdbook) covering all supported languages. Root entries also added to the monotonicity invariant test. Remaining subdirectory-only fixtures could still benefit from root-level targets.
...
- 31 test fixtures across all supported languages with per-level and budget-based snapshot tests. All 31 fixtures have budget tests; 10 fixtures have root-level tests (either, neverthrow, pluggy, sonner, commander, anyhow, log, ts-pattern, typeguard, mdbook). Monotonicity invariant tested across representative fixtures including root-level targets.
